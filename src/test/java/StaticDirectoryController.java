...
public class StaticDirectoryController {
	...
	private static boolean peekContent(String path)
	{
...
//	private static boolean isValidPathWithoutExtension(String directory, String filePath) {
...
//	}
	...
	private static boolean isValidPath(String directory, String fileExtension, String filePath) {
	    // Prevent path traversal attacks (regex + normalized path check)
	...
	private static boolean isValidPath(String file_type, String filePath)
	{
		// Prevent path traversal attacks (regex + normalized path check)
	    if (!filePath.matches("^[a-zA-Z0-9/_\\.-]+\\." + file_type + "$"))
	        return false;

	    String static_type = "static/" + file_type;
	    Path basePath = Paths.get(static_type).toAbsolutePath().normalize();
	    Path requestedPath = basePath.resolve(filePath).normalize();
	    if (!requestedPath.startsWith(basePath))
	        return false;

		Path cssPath = Paths.get(static_type, filePath).normalize();
        if (!cssPath.startsWith(Paths.get(static_type)) || !filePath.endsWith("." + file_type))
            return false;
        return true;
	}
    ...
    public ResponseEntity<byte[]> getPngFile(HttpServletRequest request) throws IOException {
        ...
        if (!isValidPath("png", filePath))
        	return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();

        // Read the CSS file as bytes
        if (peekContent("static/images/" + filePath))
...
//    }
    ...
    public ResponseEntity<byte[]> getTextFile(HttpServletRequest request) throws IOException {
        ...
        // Prevent path traversal attacks
        if (!isValidPath("txt", filePath))
        	return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();

        // Read the CSS file as bytes
        byte[] cssContent = getContent("static/texts/" + filePath);
        if (cssContent == null)
        	return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
